...
def torty_assort_pl() -> InlineKeyboardMarkup:
    ...
    for i in sql.get_pl_cakes():
        btn_assort.add(InlineKeyboardButton(f'{i[1]}', callback_data=f'tort_pl_{i[0]}'))
...
def bent_assort_pl() -> InlineKeyboardMarkup:
    ...
    for i in sql.get_pl_bent():
        btn_assort.add(InlineKeyboardButton(f'{i[1]}', callback_data=f'bent_pl_{i[0]}'))
    btn_assort.add(btn_back)
    return btn_assort
...
def mak_assortpl() -> InlineKeyboardMarkup:
    ...
    for i in sql.get_pl_mak():
        btn_assort.add(InlineKeyboardButton(f'{i[1]}', callback_data=f'mak_pl_{i[0]}'))
    btn_assort.add(btn_back)
    return btn_assort
